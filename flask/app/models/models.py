...
class ClickEvent(Event):
    ...
    __mapper_args__ = {
        'polymorphic_identity': 'ClickEvent'
    }
...
class CellClickEvent(ClickEvent):
    ...
    id = db.Column(db.Integer, db.ForeignKey('ClickEvent.id', ondelete="CASCADE"), primary_key=True)
    cell_id = db.Column(db.String(100), nullable=False)
    orig_cell_id = db.Column(db.String(100), nullable=False) # can be null or undefined when not available in the metadata

    __mapper_args__ = {
        'polymorphic_identity': 'CellClickEvent'
    }
...
class NotebookClickEvent(ClickEvent):
    ...
    __mapper_args__ = {
        'polymorphic_identity': 'NotebookClickEvent'
    }
...
class CellAlteration(Event):
    ...
    __mapper_args__ = {
        'polymorphic_identity': 'CellAlteration'
    }
...
class UserGroups(db.Model):
    ...
    group_pk = db.Column(db.String(401), primary_key=True) # combining group_name and notebook_id
